// This is my first version of tetris that I started working on in January 2020.
...
var clrdCoordsY = [
  ...
  [],
  [],
]; //clrdCoordsY stands for clrdCoordsYAxis :)
var pieceUnder = 0;
...
var cubeCoords = {
  //coords = förkortning av coordinates
  btmY: [
    [2, 2, 1, 1],
    [2, 2, 1, 1],
    [2, 2, 1, 1],
...
class Pieces {
  constructor(bottomYCoordinates, pieceColor, name) {
    //Det är constructor funktionen som runnas när jag vill creata ett nytt objekt av Brickor
  ...
  border(element, color) {
    for (randomInt = 0; randomInt < 4; randomInt++) {
      document.getElementById(element[whichForm][randomInt]).style.border =
        "1px solid " + color;
    }
  }
  ...
  borderMove() {
    if (pause == 0) {
      ...
      for (randomInt = 0; randomInt < checkCoords.length; randomInt++) {
        if (
          this.bottomCoordinatesArray[whichForm][0] == checkCoords[randomInt] ||
          this.bottomCoordinatesArray[whichForm][1] == checkCoords[randomInt] ||
          this.bottomCoordinatesArray[whichForm][2] == checkCoords[randomInt] ||
          this.bottomCoordinatesArray[whichForm][3] == checkCoords[randomInt]
        ) {
          console.log("yes");
        }
      }
  ...
  changeCoordinates(arrayAxis, yAxis) {
    for (randomInt = 0; randomInt < 4; randomInt++) {
      for (randomInt2 = 0; randomInt2 < 4; randomInt2++) {
        arrayAxis[randomInt][randomInt2] =
          "" + yAxis[randomInt][randomInt2] + this.xAxis[randomInt][randomInt2];
      }
    }
  }
  ...
  move() {
    if (pause == 0) {
      ...
      if (moveLeft == 1 && blocker == 0) {
        ...
        for (randomInt = 0; randomInt < 4; randomInt++) {
          if (
            this.xAxis[randomInt][0] !== 1 &&
            this.xAxis[randomInt][1] !== 1 &&
            this.xAxis[randomInt][2] !== 1 &&
            this.xAxis[randomInt][3] !== 1
          ) {
            for (randomInt2 = 0; randomInt2 < 4; randomInt2++) {
      ...
      if (moveRight == 1 && blocker == 0) {
        ...
        for (randomInt = 0; randomInt < 4; randomInt++) {
          if (
            this.xAxis[randomInt][0] !== 10 &&
            this.xAxis[randomInt][1] !== 10 &&
            this.xAxis[randomInt][2] !== 10 &&
            this.xAxis[randomInt][3] !== 10
          ) {
            for (randomInt2 = 0; randomInt2 < 4; randomInt2++) {
              this.xAxis[randomInt][randomInt2]++;
            }
      ...
      for (randomInt = 0; randomInt < checkCoords.length; randomInt++) {
        if (
          this.checkCoordinatesArray[whichForm][0] == checkCoords[randomInt] ||
          this.checkCoordinatesArray[whichForm][1] == checkCoords[randomInt] ||
          this.checkCoordinatesArray[whichForm][2] == checkCoords[randomInt] ||
          this.checkCoordinatesArray[whichForm][3] == checkCoords[randomInt]
        ) {
          pieceUnder = 1;
        }
      }
  ...
  switch (Math.floor(Math.random() * 7)) {
    case 0:
      moveInterval = setInterval(function () {
        cube.move();
      }, 17); //1000ms = 1 s, 1000ms/60fps = 17
      moveBorderInterval = setInterval(function () {
        cube.borderMove();
      }, 1);
    ...
    case 6:
      ...
      moveBorderInterval = setInterval(function () {
        backwardsL.borderMove();
      }, 1);
      moveYInterval = setInterval(function () {
        backwardsL.decreaseYAxis();
      }, moveDownInt);
      break;
  }
}
